...
def analyze_case(root: Path, target_adsorption: float, target_dband: float | None) -> dict[str, object]:
...
def analyze_cases(paths: list[Path], target_adsorption: float, target_dband: float | None) -> dict[str, object]:
    cases = [analyze_case(path, target_adsorption, target_dband) for path in paths]
    ...
    return {
        ...
        "ranking_basis": "screening_score = |E_ads - target| + forward_barrier + 0.25*|d_band_center - target_dband|",
        "cases": ranked,
...
def main() -> None:
    ...
    payload = analyze_cases([Path(path).expanduser().resolve() for path in args.paths], args.target_adsorption, args.target_dband)
    if args.json:
